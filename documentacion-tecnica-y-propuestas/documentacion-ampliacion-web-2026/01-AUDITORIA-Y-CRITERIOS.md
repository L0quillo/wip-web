...
En la raíz no existe otra web HTML independiente. La raíz contiene material histórico, fotografías, logos y documentos fuente. La web desplegable actual está en `sitio-web-actual/wip-new/`; los ZIP fechados se conservan en `respaldos-versiones-web/` y `sitio-web-actual/redirect-root/` contiene la redirección opcional.
